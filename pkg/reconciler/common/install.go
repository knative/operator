...
import (
	...
	mff "github.com/manifestival/manifestival/pkg/filter"
	"knative.dev/operator/pkg/apis/operator/v1alpha1"
	"knative.dev/pkg/logging"
)

var (
	role        mff.Predicate = mff.Any(mff.ByKind("ClusterRole"), mff.ByKind("Role"))
	rolebinding mff.Predicate = mff.Any(mff.ByKind("ClusterRoleBinding"), mff.ByKind("RoleBinding"))
)
...
func Install(ctx context.Context, manifest *mf.Manifest, instance v1alpha1.KComponent) error {
	...
	if err := manifest.Filter(mff.Not(mff.Any(role, rolebinding))).Apply(); err != nil {
		status.MarkInstallFailed(err.Error())
...
func Uninstall(manifest *mf.Manifest) error {
	if err := manifest.Filter(mff.NoCRDs, mff.Not(mff.Any(role, rolebinding))).Delete(); err != nil {
	...
	if err := manifest.Filter(mff.Any(role, rolebinding)).Delete(); err != nil {
