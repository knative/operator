...
func verifyDefaultConfig(t *testing.T, ks *v1alpha1.KnativeServing, defaultsConfigMapName string, clients *test.Clients, names test.ResourceNames) {
	...
	err = WaitForConfigMap(defaultsConfigMapName, clients.Kube, func(m map[string]string) bool {
...
func verifyLoggingConfig(t *testing.T, loggingConfigMapName string, clients *test.Clients, names test.ResourceNames) {
	err := WaitForConfigMap(loggingConfigMapName, clients.Kube, func(m map[string]string) bool {
...
func verifySingleKeyDeletion(t *testing.T, loggingConfigKey string, loggingConfigMapName string, clients *test.Clients, names test.ResourceNames) {
	ks, err := clients.KnativeServing().Get(context.TODO(), names.KnativeServing, metav1.GetOptions{})
	if err != nil {
		t.Fatal("failed to get knative serving: ", err)
	} else if ks.Spec.Config[loggingConfigKey]["loglevel.autoscaler"] == "" {
		t.Fatal("Existing KS operator CR lacks proper key: ", ks.Spec.Config)
	}
...
func DeleteAndVerifyDeployments(t *testing.T, clients *test.Clients, names test.ResourceNames) {
	dpList, err := clients.Kube.AppsV1().Deployments(names.Namespace).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
	...
	if err := clients.Kube.AppsV1().Deployments(deployment.Namespace).Delete(context.TODO(), deployment.Name, metav1.DeleteOptions{}); err != nil {
		t.Fatalf("Failed to delete deployment %s/%s: %v", deployment.Namespace, deployment.Name, err)
	}

	waitErr := wait.PollImmediate(Interval, Timeout, func() (bool, error) {
		dep, err := clients.Kube.AppsV1().Deployments(deployment.Namespace).Get(context.TODO(), deployment.Name, metav1.GetOptions{})
		if err != nil {
